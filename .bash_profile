...
alias docker-pull-all='docker images --format "{{.Repository}}:{{.Tag}}" | grep -v "<none>" | sort | uniq | xargs -L1 docker pull'
alias aws-assume='gossamer -c ~/aws-roles-assume.yml'
alias openshift-auth='docker run -it --rm openshift-auth:latest'
